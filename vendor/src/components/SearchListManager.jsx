...
const FamilySearchSection = ({ family, allProducts, onProductsChange, onEditProduct, readOnly = false }) => {
...
const SearchListManager = ({ onEditProduct, readOnly = false }) => {
      ...
      <div className="bg-white rounded-[2rem] border border-gray-100 shadow-xl p-6 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <p className="text-sm text-gray-500">
            {readOnly
              ? 'Browse each product family, its filters, and matching products on the website.'
              : 'Each'}{' '}
            {!readOnly && (
              <>
                <strong>3rd sub-category</strong> has one fixed filter set shared by all its
