...
import { Loader2, Mail, X, CheckCircle, Circle, MessageSquare } from 'lucide-react';
...
export default function Inquiries() {
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get('/vendors/inquiries');
  ...
  const openDetail = async (item) => {
    ...
    if (item.status === 'new') {
      try {
        await api.patch(`/vendors/inquiries/${item._id}`, { status: 'read' });
        setItems((prev) =>
    ...
    <div className="p-8 lg:p-12 max-w-[1400px] mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Customer inquiries for your products from the Nexyos website.
          {unreadCount > 0 && (
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center text-gray-500">
                    No inquiries for your products yet.
                  </td>
                </tr>
              ) : (
                items.map((item) => (
